...
func (c *MainController) GoodsGet(){
	o := orm.NewOrm()
...
func Select(ids  string) (string, string, string){
	...
	sqlText2 := fmt.Sprintf("select id, name,create_time  from manager where name is not null")
...
func (c *MainController) Get() {
	...
	if err1 == nil && err2 == nil && err3 == nil{
		for _, i := range maps2{
			result = append(
				result, map[string]string{"id" : i["id"].(string), "name" : i["name"].(string), "mai" : "0", "ti" : "0", "price" : "0", "date" : date, "riqi": i["create_time"].(string)},
			)
